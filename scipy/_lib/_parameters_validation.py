...
def validate_params(parameter_constraints, *, prefer_skip_nested_validation):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                skip_parameter_validation = prefer_skip_nested_validation
                # with config_context(
...
class StrOptions(Options):
    ...
    def __init__(self, options, *, deprecated=None, case_sensitive=False):
        super().__init__(type=str, options=options, deprecated=deprecated)
        self.case_sensitive = case_sensitive

    def is_satisfied_by(self, val: str | None):
        val = val if self.case_sensitive or val is None else val.lower()
        return isinstance(val, self.type) and val in self.options
